...
export function sanitizeSettings(partial: Partial<AppSettings>): AppSettings {
  ...
  const popoverShortcut = merged.popover_shortcut.trim() || DEFAULT_SETTINGS.popover_shortcut
  const hotkeyTranslateShortcut =
    merged.hotkey_translate_shortcut.trim() || DEFAULT_SETTINGS.hotkey_translate_shortcut
  const audioMode = merged.auto_play_audio_mode || DEFAULT_SETTINGS.auto_play_audio_mode
  const triggerMode = merged.popover_trigger_mode || DEFAULT_SETTINGS.popover_trigger_mode
  const panelMode = merged.popover_open_panel_mode || DEFAULT_SETTINGS.popover_open_panel_mode
  const languageMode =
    merged.popover_definition_language_mode || DEFAULT_SETTINGS.popover_definition_language_mode
