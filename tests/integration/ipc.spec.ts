...
describe('Tauri IPC services', () => {
  ...
  it('calls translate command with payload', async () => {
    ...
    mockInvoke.mockResolvedValue({ result: 'Hello', engine: 'argos', mode: 'direct' })

    const result = await translateText({ text: 'xin chào', source: 'vi', target: 'en' })

    expect(mockInvoke).toHaveBeenCalledWith(
      'translate_text',
      expect.objectContaining({
        payload: expect.objectContaining({ text: 'xin chào', source: 'vi', target: 'en' })
      })
    )
    expect(result).toEqual({ result: 'Hello', engine: 'argos', mode: 'direct' })
  })
  ...
  it('calls dictionary lookup command with payload', async () => {
    ...
    mockInvoke.mockResolvedValue({
      ...
      phonetic: null,
      audio_url: null,
      meanings: [{ part_of_speech: 'noun', definitions: ['hello'], example: null }],
